...
#Display the product details as per the following criteria and sort them in descending order of category:
  ...
  P.PRODUCT_DESC, PC.PRODUCT_CLASS_CODE, PC.PRODUCT_CLASS_DESC
FROM PRODUCT P
JOIN PRODUCT_CLASS PC ON P.PRODUCT_CLASS_CODE = PC.PRODUCT_CLASS_CODE
ORDER BY PC.PRODUCT_CLASS_CODE DESC;

#List the product description, class description and price of all products which are shipped
...
SELECT 
    ...
    PC.PRODUCT_CLASS_DESC AS 'Product Class',
    P.PRODUCT_PRICE AS 'Product Price',
    P.PRODUCT_QUANTITY_AVAIL AS 'Available Quantity',
    CASE 
        WHEN (P.PRODUCT_CLASS_CODE = 1 OR P.PRODUCT_CLASS_CODE = 2) AND P.PRODUCT_QUANTITY_AVAIL = 0 THEN 'Out of stock'
        WHEN (P.PRODUCT_CLASS_CODE = 1 OR P.PRODUCT_CLASS_CODE = 2) AND P.PRODUCT_QUANTITY_AVAIL < 10 THEN 'Low stock'
        WHEN (P.PRODUCT_CLASS_CODE = 1 OR P.PRODUCT_CLASS_CODE = 2) AND P.PRODUCT_QUANTITY_AVAIL BETWEEN 11 AND 30 THEN 'In stock'
        WHEN (P.PRODUCT_CLASS_CODE = 1 OR P.PRODUCT_CLASS_CODE = 2) AND P.PRODUCT_QUANTITY_AVAIL > 31 THEN 'Enough stock'
        WHEN (P.PRODUCT_CLASS_CODE = 3 OR P.PRODUCT_CLASS_CODE = 4) AND P.PRODUCT_QUANTITY_AVAIL = 0 THEN 'Out of stock'
        WHEN (P.PRODUCT_CLASS_CODE = 3 OR P.PRODUCT_CLASS_CODE = 4) AND P.PRODUCT_QUANTITY_AVAIL < 20 THEN 'Low stock'
        WHEN (P.PRODUCT_CLASS_CODE = 3 OR P.PRODUCT_CLASS_CODE = 4) AND P.PRODUCT_QUANTITY_AVAIL BETWEEN 21 AND 80 THEN 'In stock'
        WHEN (P.PRODUCT_CLASS_CODE = 3 OR P.PRODUCT_CLASS_CODE = 4) AND P.PRODUCT_QUANTITY_AVAIL > 81 THEN 'Enough stock'
...
SELECT c.CUSTOMER_ID, c.CUSTOMER_FNAME, c.CUSTOMER_LNAME, c.CUSTOMER_EMAIL, c.CUSTOMER_PHONE, a.COUNTRY
FROM online_customer c
INNER JOIN address a ON c.ADDRESS_ID = a.ADDRESS_ID
WHERE a.STATE != 'Karnataka' AND c.CUSTOMER_ID NOT IN 
(SELECT oh.CUSTOMER_ID FROM order_header oh
INNER JOIN order_items oi ON oh.ORDER_ID = oi.ORDER_ID
INNER JOIN product p ON oi.PRODUCT_ID = p.PRODUCT_ID
INNER JOIN product_class pc ON p.PRODUCT_CLASS_CODE = pc.PRODUCT_CLASS_CODE
WHERE pc.PRODUCT_CLASS_DESC IN ('Toys', 'Books'))
